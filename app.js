...
(function () {
  ...
  /* This script's own ?v=<VERSION> (see index.html and sw.js), reused to
     build map.js's URL when it's lazily inserted, so the two always match
     without a second hardcoded version string. */
  var SELF_SRC = (document.currentScript && document.currentScript.src) || '';
  var VERSION_Q = (SELF_SRC.match(/\?v=([^&]+)/) || [])[1] || '';

  var DATA_URL = './data/kitchens.json';
  var DISHES_URL = './data/dishes.json';
  /* Map view (see "Map (lazy-loaded engine)" below): map.js owns its own
     coordinate frame and file URLs now; only the quadrant title text stays
     here since the "not ready yet" / skeleton path needs it before map.js
     has loaded. */
  ...
  var state = {
    ...
    map: {
      ...
      servesPaths: [],
      /* The map view was hidden (list mode, or another page) since the
         last zoom: the next zoom jumps to place instead of animating (read
         and cleared by map.js's renderMap). */
      justShown: false
    }
  };
  var dom = {};
  ...
  /* The prices a kitchen actually has, in a fixed order:
     ...
     and priced. */
  /* perDay divides each recurring plan back to a daily rate (week / 7,
     month / 30) purely to compare the kitchen's own prices against each
     other -- never shown on its own. best marks the row with the strictly
     lowest perDay, only with 2+ recurring plans and no tie, so "Best
     value" is always a plain fact from the kitchen's own prices. Trial is
     excluded: a one-time taster, not a recurring plan. */
  ...
  function applyMode() {
    ...
    if (dom.mapView) {
      ...
      if (dom.mapView.hidden && !hide) state.map.justShown = true;
  ...
  /* ---------------------------------------------------------------------
     Map (lazy-loaded engine, ./map.js)
     ?view=map: an illustrated SVG (data/map/calgary.json, airdrie.json)
     with pickup pins, clustering and a preview card. All of that lives in
     map.js (Round 40), fetched with a same-origin <script> the first time
     the map is actually needed -- never on first load. Until then, or if
     it never loads (offline), the map shows its own skeleton/error pane,
     kept here since they don't need the engine.
     Once loaded, map.js registers window.TFMap and reads/writes shared
     state through window.TF (the bridge built below). The functions here
     sharing map.js's old names (loadMap, renderMap, onMapStageClick,
     onMapReset, retryMap, onMapCardEnter/Leave, closeMapCardNow,
     onMapResize) are what the rest of app.js already calls or wires as
     listeners; they either handle the map-less state themselves (onMapReset,
     retryMap need no engine) or delegate to window.TFMap once it exists. */
  ...
  var mapStylesPromise = null;

  /* A timing token from styles.css (such as --dur-2) in milliseconds. Also
     used by map.js's own copy of this same tiny helper (duplicated there
     rather than bridged, like fmt/afterPaint). */
  ...
  /* Insert <link rel="stylesheet" href="./map.css?v=..."> once, the first
     time the map is needed, alongside the map.js <script> (ensureMapScript
     below) -- the pin/area/card drawing rules map.js's own renderMap needs
     (see the "map.css" note in styles.css's "9d. Map" section). Resolves
     once the stylesheet has loaded (or errors quietly and resolves anyway:
     a missing stylesheet must not block the script from still trying, and
     an unstyled map is a much smaller problem than a broken one). */
  function ensureMapStyles() {
    if (mapStylesPromise) return mapStylesPromise;
    mapStylesPromise = new Promise(function (resolve) {
      var link = document.createElement('link');
      link.rel = 'stylesheet';
      link.href = './map.css' + (VERSION_Q ? '?v=' + VERSION_Q : '');
      link.addEventListener('load', function () { resolve(); });
      link.addEventListener('error', function () { resolve(); });
      document.head.appendChild(link);
    });
    return mapStylesPromise;
  }

  /* Insert <script src="./map.js?v=<same version app.js loaded with>">
     once, the first time the map is needed. Resolves with window.TFMap
     once it (and its stylesheet) have loaded and it has registered
     itself; every caller shares the one promise, so a second "open the
     map" tap while it's still loading doesn't insert a second script
     tag. */
  function ensureMapScript() {
    if (window.TFMap) return Promise.resolve(window.TFMap);
    if (mapScriptPromise) return mapScriptPromise;
    mapScriptPromise = Promise.all([ensureMapStyles(), new Promise(function (resolve, reject) {
      var s = document.createElement('script');
      s.src = './map.js' + (VERSION_Q ? '?v=' + VERSION_Q : '');
      s.addEventListener('load', function () {
        if (window.TFMap) resolve(window.TFMap);
        else reject(new Error('map.js loaded but did not register window.TFMap'));
      });
      s.addEventListener('error', function () {
        reject(new Error('map.js failed to load'));
      });
  ...
  /* Ensures map.js, then hands off to its loadMap(). A script-load failure
     (no connection, blocked request) is treated the same as a fetch
     failure: the map's own error/offline pane. */
  function loadMap() {
    ensureMapScript().then(function (TFMap) {
      TFMap.load();
    }).catch(function () {
      state.map.status = 'error';
      state.map.offline = navigator.onLine === false;
      if (parseRoute().view === 'browse' && state.mode === 'map') renderResults();
    });
  }

  /* Draw the map for the filtered list (called by renderResults in map
     mode). f is the filter set; f.quadrant picks the zoom. The title,
     reset button and "not ready yet" states never need the engine, so they
     stay here; the actual drawing (once map.js is loaded and the files are
     in) is window.TFMap.render. */
  function renderMap(list, f) {
    ...
    if (window.TFMap) window.TFMap.render(list, f, key);
  }

  /* Tap part of the map (with "All" quadrants on) to zoom to its quadrant:
     the same as picking that quadrant above. Needs the engine (the pin
     click it must ignore, the zoom areas themselves), so it just delegates
     once map.js is loaded -- it can't be tapped before then anyway, since
     the map itself isn't drawn. */
  function onMapStageClick(event) {
    if (window.TFMap) window.TFMap.onStageClick(event);
  }

  /* "Show all of Calgary": back to every quadrant. The button hides, so
     focus lands on the count, which announces the new result. Pure filter
     logic -- no map engine involved. */
  function onMapReset() {
  ...
  function retryMap() {
    if (state.map.status === 'loading') return;
  ...
  function onMapCardEnter(event) {
    if (window.TFMap) window.TFMap.onCardEnter(event);
  }

  function onMapCardLeave(event) {
    if (window.TFMap) window.TFMap.onCardLeave(event);
  }

  /* Nothing to close before map.js has ever built a card. */
  function closeMapCardNow() {
    if (window.TFMap) window.TFMap.closeCardNow();
  }

  function onMapResize() {
    if (window.TFMap) window.TFMap.onResize();
  }

  /* The small delivery-area map on a kitchen's own page (renderKitchen):
     ensures map.js/map.css, then hands off to TFMap.renderDeliveryPreview.
     Quiet on failure (offline, first load with no connection) -- the
     accessible text list of areas above it already says the same thing,
     so an empty container is a fine fallback, not an error to show. */
  function loadDeliveryMap(container, k) {
    ensureMapScript().then(function (TFMap) {
      TFMap.renderDeliveryPreview(container, k);
    }).catch(function () { /* text list above already covers it */ });
  }
  ...
  function renderKitchen(slug) {
    ...
    planPrices(k).forEach(function (p) {
      ...
      var rowHead = el('th', { scope: 'row', text: p.label });
      if (p.best) {
        rowHead.appendChild(document.createTextNode(' '));
        rowHead.appendChild(el('span', { class: 'tag tag-best', text: 'Best value' }));
      }
      tbody.appendChild(el('tr', p.best ? { class: 'is-best' } : null, [rowHead, el('td', { text: money(p.amount) })]));
    });
    if (k.trial) {
  ...
  /* The bridge map.js talks to once it loads (see "Map (lazy-loaded
     engine)" above): state and dom by reference (mutations either file
     makes are visible to both, same objects throughout), plus the render
     helpers map.js's pin/card/quadrant code reuses from the rest of
     app.js instead of duplicating them. Built once, here -- every name in
     it is a function declaration (hoisted) or a var already assigned by
     the time boot() runs, so its position doesn't matter beyond "after
     they're all defined, before anything could need it" (map.js is never
     requested before boot() wires up the List/Map switch). */
  window.TF = {
    state: state,
    dom: dom,
    el: el,
    ...
    trialChip: trialChip,
    sampleTag: sampleTag,
    businessTypeChip: businessTypeChip,
    dabbaTile: dabbaTile,
    bagGlyph: bagGlyph,
    tiffinGlyph: tiffinGlyph,
