...
### Nutrition (new, optional, kitchen-provided)
...
"nutrition": {
  ...
  "protein_min_g": 25,
  "protein_max_g": 30,
  "contains": ["milk", "wheat and triticale"],
  "notes": "Mild spice, vegetarian option available.",
  "estimated_on": "2026-11-01",
  "method": "kitchen estimate"
}
```

Entirely optional — most real kitchens won't fill this in, and nothing is
shown when they don't. When a kitchen does provide it, Tiffin Finder shows
it as a quiet, clearly-labelled panel near "Plans and prices": a calorie
range, a protein range, an allergen list, the method and date, and a fixed
disclaimer (see below). Ranges only, never a single precise number — a
home kitchen's own batches vary too much for that.

- `per` — the unit the range is for. Only `"meal"` is understood today; the
  whole object is dropped if it's anything else or missing (it defaults to
  `"meal"` when left out).
- `calories_min` / `calories_max` — whole numbers, `calories_min > 0`,
  `calories_max >= calories_min`, both `<= 3000`. Either both are present
  or neither is.
- `protein_min_g` / `protein_max_g` — whole grams, `protein_min_g >= 0`,
  `protein_max_g >= protein_min_g`, both `<= 250`. Either both are present
  or neither is. At least one of the calorie or protein range is required
  for the panel to show at all.
- `contains` — a list drawn only from Canada's 11 priority allergens, using
  these exact strings: `peanuts`, `tree nuts`, `sesame`, `milk`, `eggs`,
  `fish`, `crustaceans and molluscs`, `soy`, `wheat and triticale`,
  `mustard`, `sulphites`. Anything else in the list is dropped rather than
  shown. This is a courtesy tag from the kitchen, never a guarantee — the
  panel's disclaimer says so.
- `notes` — up to 200 characters of short, factual description (for
  example "Mild spice, vegetarian option available"). **Never** a
  nutrient-content or health claim — see "Banned nutrition and health
  claims" below. A note that contains a banned phrase is dropped, not
  shown with the phrase edited out.
- `estimated_on` — `YYYY-MM-DD`, the day the kitchen worked this out. Not
  required, but shown next to the method when present.
- `method` — one of `"kitchen estimate"`, `"recipe calculator"` or
  `"dietitian"`. Shown as a short line ("Kitchen estimate", "Recipe
  calculator", "Dietitian reviewed").

**Banned nutrition and health claims** (see `plans/nutrition-research.md`
for the legal reasoning): none of these words or phrases may appear
anywhere that describes a kitchen or its food — not in `nutrition.notes`,
not in `description`, not anywhere else in `data/kitchens.json` — even
quoted, negated, or as part of a longer word:

> low fat, low-fat, high protein, high-protein, healthy, keto, diabetic,
> low sodium, low carb, heart-healthy, nut-free, allergen-free, guaranteed

These are nutrient-content claims ("low fat", "high protein") and health or
lifestyle claims ("healthy", "keto", "diabetic") that Health Canada/CFIA
regulate with specific numeric thresholds or evidence requirements a home
kitchen can't easily meet, plus absolute allergen promises
("nut-free", "allergen-free", "guaranteed") that a shared or home kitchen
can't credibly make. `tools/check_listings.py` scans the whole file for
them and fails the build if any appear; `guide.html` explains the policy
to households without repeating the banned words.

**Sample kitchens**: about a third of the 24 sample kitchens carry
illustrative `nutrition` data, generated by `tools/sample_nutrition.py`
(deterministic, consistent with each kitchen's `veg_only`/`halal` flags and
its dishes' own `contains` tags in `data/dishes.json`). The panel always
says "Sample estimate" for these, on top of the listing's existing "Sample
listing" tag.
...
- `nutrition`, if present, is shaped correctly (ranges, allergen names,
  method, date) and its `notes` carries no banned nutrition/health claim
- none of the banned phrases ("AHS approved", "verified by", "Permit
  verified") appear anywhere in the file
- none of the banned nutrition/health claim phrases (see "Banned nutrition
  and health claims" above) appear anywhere in the file
